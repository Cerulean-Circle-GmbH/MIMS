...
function checkAndCreateDataVolume() {
  ...
  if [[ $datavolume == *"/"* ]]; then
    ...
    SCENARIO_DATA_MOUNTPOINT="nagios-volume"
  ...
  if [ "$SCENARIO_DATA_RESTORESOURCE" != "none" ]; then
    ...
    if [[ $SCENARIO_DATA_VOLUME == *"/"* ]]; then
      ...
      if [ "$(ls -A $SCENARIO_DATA_VOLUME)" ]; then
        ...
        # Extract data and strip /var/nagios_home from the tar
...
function test() {
  ...
  if [ "$VERBOSITY" = "-v" ]; then
    banner "Test"
    log "Volumes:"
    docker volume ls | grep ${SCENARIO_NAME}_nagios_home
    log "Images:"
  ...
  checkURL "Nagios (http)" http://$SCENARIO_SERVER_NAME:$SCENARIO_RESOURCE_HTTPPORT/nagios
